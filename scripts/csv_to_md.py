...
导入只是「搭好骨架」——之后应按 references/resume.md 的规则完成事实核验和改写。
    ...
    if suffix == ".xlsx":
        ...
        for encoding in ("utf-8-sig", "gb18030"):
            try:
                ...
                continue
        if decoded is None:
            sys.exit("[csv_to_md] CSV 编码无法识别；请另存为 UTF-8 或 GB18030。")
        return [row for row in csv.reader(decoded.splitlines()) if row]
...
def build(rows):
    ...
    for row in rows:
        ...
        if target == "__summary__":
            item = value.strip()
            if item:
                summary_parts.append(f"{field}：{item}" if field else item)
            continue
        if target is None:
            # 未知分类：当作一般经历节，用原分类名
            target = base
    ...
    return meta, intent_parts, "\n".join(summary_parts), groups
